...
def obtenir_type_horloge():
    while True:
        type_horloge = input("Entrez le type d'affichage de l'heure (AM-PM/24H) : ").upper()
        if type_horloge in ['AM-PM', '24H']:
            return type_horloge
        else:
            print("Veuillez entrer une option valide (AM-PM/24H).")
...
def afficher_heure(heure_base, heure_alarme, est_en_pause):
    ...
    while True:
        ...
        alarme(heure_alarme, minute_alarme, seconde_alarme, heure, minute, seconde)  
        temps_actuel = time()
